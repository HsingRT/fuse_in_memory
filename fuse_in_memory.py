...
class InMemoryFileSystem(Operations):
    ...
    def readdir(self, path, fh):
        return ['.', '..'] + [
            ...
            if name != '/' and os.path.dirname(name) == path
        ]
    ...
    def set_key(self, path, key):
        if len(key) != 32:
            raise ValueError(
                "The encryption key must be 32 bytes (256 bits) long.")
        self.keys[path] = key
    ...
    def write(self, path, data, offset, fh):
        ...
        if path not in self.keys:
            raise FuseOSError(errno.EACCES)
        decrypted_data = self.decrypt(self.keys[path], self.data[path])
        new_data = decrypted_data[:offset] + data + \
            decrypted_data[offset + len(data):]
